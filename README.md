...
`eslee OneKey`는 Windows에서 단축키 또는 프로세스 시작을 감지해 오디오 출력 장치를 전환하는 데스크톱 앱입니다. 감시 프로세스가 종료된 뒤에는 기본적으로 현재 장치를 그대로 유지하며, 옵션을 켜면 원래 장치로 안전하게 복원합니다. 사용자가 세션 도중 직접 오디오 장치를 바꾸면 그 선택을 우선합니다.

## MVP 기능

- 시스템 전역 단축키와 감시 프로세스 시작 감지
- 현재 기본 출력 장치 저장 후 지정 헤드셋으로 전환
- 종료 후 오디오 처리 선택: 현재 장치 유지(기본) 또는 원래 장치 자동 복원
- 게임과 Discord 중복 실행 방지(이미 실행 중이면 창을 전면으로 가져오지 않음)
- 자동 복원을 켠 경우 종료 시 Discord 음성 상태 API 확인
